...
if __name__ == '__main__':
    ...
    lz_finder = ot.LzFinder(model_obj_det=MODEL_OBJ_DET, model_seg=MODEL_SEG, labels=LABELS_YOLO, res=RES,
                            label_list=CLASSES, use_seg_for_lz=False, r_landing_factor=8, stride=75)
    print(lz_finder)
    ...
    while drone_is_one:
        ...
        rc_values = tp.keyboard_rc(drone, rc_values, py_game, drone.MANUAL_SPEED)
        ...
        if battery_level <= 15:
            ...
            py_game.display_multiple_status(screen=screen, screen_variables_names_units=screen_variables_names_units,
                                            ...
                                            temperature=temperature, speed=drone.MANUAL_SPEED, flight_phase=drone.flight_phase,
                                            auto_pilot_armed=auto_pilot.auto_pilot_armed)
        pygame.display.flip()
